...
/**
 * Perceptual hash (dHash) for cross-device image recognition. Produces a 64-bit hash based on image
 * content, not metadata. Same image on different devices → same hash.
 */
object ImageHasher {
  /**
  ...
  /**
   * Compute hamming distance between two hashes. Lower = more similar. 0 = identical. Typically <
   * 10 means same image with minor differences.
   */
  fun hammingDistance(
      hash1: String,
      hash2: String,
