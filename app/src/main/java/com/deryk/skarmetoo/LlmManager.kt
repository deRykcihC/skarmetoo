...
class LlmManager(private val context: Context) {
    ...
    scope.launch {
      engineMutex.withLock {
        try {
          ...
          val mainBackend =
              if (isGemma4) Backend.CPU() else if (useGpu) Backend.GPU() else Backend.CPU()
          ...
          engine = newEngine
          engineConfigForPool = engineConfig
  ...
  /**
   * Analyze a screenshot and return structured summary + tags. Closes existing conversation first
   * (engine only supports 1 session). Asks model to output JSON, with fallback parsing.
   */
...
TAGS: [extracted tag1, tag2, tag3]"""
  ...
  private fun parseAnalysisResult(result: String): Pair<String, String> {
    ...
    val summaryMatch =
        Regex("(?i)SUMMARY\\s*:\\s*(.*?)(?=\\s*(?:TAGS\\s*:|$))", RegexOption.DOT_MATCHES_ALL)
            .find(cleaned)
    ...
    if (summary.isBlank()) {
      ...
      if (summary.isBlank()) {
        val altSummaryMatch =
            Regex("(?i)\"(?:description|content|caption)\"\\s*:\\s*\"(.*?)\"").find(cleaned)
  ...
  /**
   * Analyze a screenshot using a dedicated engine from the pool (no mutex blocking). This allows
   * true concurrent analysis with multiple instances.
   */
  fun analyzeScreenshotConcurrent(
    ...
    scope.launch {
      ...
      try {
        ...
        val conv = poolEngine.createConversation(convConfig)
        Log.d(TAG, "Created pool conversation for concurrent analysis")
...
TAGS: [extracted tag1, tag2, tag3]"""
        ...
        val fullResponse = StringBuilder()
        Log.d(TAG, "Pool engine: sending image for analysis...")
