...
class LlmManager(private val context: Context) {
    ...
    fun initializeModel(modelPath: String, useGpu: Boolean = false, maxTokens: Int = 4096) {
    ...
    fun analyzeImage(bitmap: Bitmap, prompt: String) {
        ...
        scope.launch {
            try {
                ...
                val argbBitmap = if (resized.config != Bitmap.Config.ARGB_8888) {
        ...
        scope.launch {
            engineMutex.withLock {
                try {
                    ...
                    val conv = engine?.createConversation(convConfig) ?: run {
                    ...
                    val prompt = when (detailLevel) {
                        DetailLevel.BRIEF -> """Describe this screenshot briefly. Respond with EXACTLY this format and nothing else:
SUMMARY: [your one sentence description]
TAGS: [tag1, tag2, tag3]"""

                        DetailLevel.DETAILED -> """Describe this screenshot in detail. Write 2-3 sentences covering what is shown, visible text, UI elements, and purpose. Respond with EXACTLY this format and nothing else:
SUMMARY: [your detailed 2-3 sentence description]
TAGS: [tag1, tag2, tag3, tag4, tag5]"""

                        DetailLevel.COMPREHENSIVE -> """Describe this screenshot with maximum detail in a single paragraph. Cover ALL visible text, UI elements, colors, layout, branding, and objects. Omit nothing. Respond with EXACTLY this format and nothing else:
SUMMARY: [your comprehensive paragraph describing absolutely everything visible in the image]
TAGS: [tag1, tag2, tag3, tag4, tag5, tag6, tag7, tag8]"""

                        DetailLevel.CUSTOM -> """${customPrompt ?: "Describe this screenshot."}
Respond with EXACTLY this format and nothing else:
SUMMARY: [your response based on the instruction]
TAGS: [extracted tag1, tag2, tag3]"""
                    }

                    val estimatedTotalChars = when (detailLevel) {
                    ...
                    val resultPair = kotlin.coroutines.suspendCoroutine<Pair<String, String>?> { continuation ->
                        var isResumed = false
                        try {
     ...
     */
    private fun extractTags(json: org.json.JSONObject, key: String): String {
