...
/**
 * Empty activity that appears in the share drawer. When selected, it confirmed to the user that the
 * image is saved. (In this app, the image is physically saved to the MediaStore before opening the
 * drawer to get the Uri, so this activity just confirms that storage location as a discrete
 * action).
 */
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    if (streamUri != null) {
      ...
      android.media.MediaScannerConnection.scanFile(
          this, arrayOf(getRealPathFromURI(streamUri)), null) { path, uri ->
